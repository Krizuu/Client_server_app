...
{
    /**
     * Start the server and creats threads
     */
    private Thread t;
    ...
    /**
     *  list of files owned or shared to user
     * @param username  name
     * @return  list of files of users
     */
    ...
    /**
     * adding files from Collection f to global file list
     * @param f files add to list
     * @return same
     */
    ...
    //TESTS
    public static int SaveCSV(String filename, String user, long filesize, String drive, String directory){
        try {
            BufferedWriter fileWriter = new BufferedWriter(new FileWriter(directory + "/" + drive + "/" + drive + ".csv", true));
            synchronized (fileWriter) {
                ...
                fileWriter.append(",");
                fileWriter.append(String.valueOf(filesize));
                fileWriter.newLine();
                fileWriter.flush();
                fileWriter.close();
            }
        }catch (IOException e)
        {
            ...
            //System.out.println("Błąd z zapisem do pliku csv");
            return 1;
        }
        return 0;
    }
    ...
    static class FileListUpdater implements Runnable
    {
        ...
        private ArrayList<FileEntry> createListHdd()
        {
            ...
            for(int hddNo = 1; hddNo <= 4; hddNo+=1)
            {
                ...
                for(String fPath : listHdd)
                {
                    ...
                    else
                    {
                        ...
                        filesHdd.add(new FileEntry(file.getName(), hddNo, file.getPath().replace("\\", "\\\\"), size, split[2]));
                    }
                }
            }
            return filesHdd;
